#Получить поперечные распределения интенсивности светового поля в фокусе (z=0)
#апланатического объектива при фокусировке радиально- поляризованной моды R-TEM01
#(параметр β считать равным 1) с длиной волны 0,532 мкм в воздухе (n = 1).
#Вычисления произвести для значений числовой апертуры NA равной 0,65, 0,8 и 0,95.
#Повторить вычисления для параметра β равного 0,5, 1,5, 2,0.

lambda = 0.532; % длина волны в мкм
k = 2.0 * pi / lambda; % волновое число
NA = 0.65; % значение числовой апертуры
betta = 1.5;
n = 1; % воздух
...
colorbar;
axis xy;
title('Распределение интенсивности при NA = 0.65 и betta = 1.5');
xlabel('x, мкм');
ylabel('y, мкм');
